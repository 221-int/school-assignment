...
#     )
#
#     # ── 2. 탐지용: 침식(경계 노이즈 제거) 후 CLOSE(구멍 메우기) ──
...
#         cv2.imwrite("test_image/debug_blue_closed_strict.png", blue_closed_strict)
#         cv2.imwrite("test_image/debug_blue_closed_wide.png", blue_closed_wide)
...
#
#         if not (3 < area < 300 and cw_ < lx * 0.5 and ch_ < h * 0.15):
#             if debug:
...
#
#         if blue_ratio > 0.5:  # 0.4 → 0.3으로 완화
...
#         return image
#
#     # ── 8. final_mask 내 검정 픽셀만 파랑으로 채운 후 inpainting ──
#     gray_tmp = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
#     dark_pixels = (gray_tmp < 20).astype(np.uint8) * 255
#     dark_in_ref = cv2.bitwise_and(dark_pixels, final_mask)
#
...
def remove_background_noise(image, left_ratio=0.15, debug=False):
    ...
    # ── 8. 노이즈 픽셀을 주변 로컬 파란 평균색으로 직접 덮기 ──
    # inpainting 대신 각 컴포넌트 주변 파란 픽셀의 평균으로 채움
    # → 멀리서 픽셀을 끌어오는 문제 방지
    result = image.copy()

    # final_mask의 각 연결 컴포넌트별로 주변 파란 픽셀 평균 계산
    num_final, final_labels, final_stats, _ = cv2.connectedComponentsWithStats(
    ...
    for i in range(1, num_final):
        ...
        if len(local_blue_pixels) > 0:
            ...
            # 로컬에 파란 픽셀 없으면 전체 search_zone 평균 사용
