...
# remove_background_noise_test 호출 전에 추가
image_check = cv2.imread("test_image/im078-pneumonia.jpg")
hsv_check = cv2.cvtColor(image_check, cv2.COLOR_BGR2HSV)

# R 마커가 있을 것 같은 좌측 중간 영역 샘플링
print("=== R 마커 위치 HSV 샘플링 ===")
for y in range(100, 200, 5):
    for x in range(5, 40, 5):
        h_val, s_val, v_val = hsv_check[y, x]
        print(f"  ({y},{x}) H={h_val}, S={s_val}, V={v_val}")
...
def remove_background_noise_test(image, left_ratio=0.15, debug=False):
    ...
    # ── 5. 파란 배경과 색상이 다른 픽셀 탐지 (수정됨) ──
    # 기존의 복잡한 H, S 조건 제거 (하얀색/저채도 노이즈 누락 방지)
    not_blue_mask = cv2.bitwise_not(blue_mask)
    candidate_mask = cv2.bitwise_and(not_blue_mask, search_zone)
    ...
    # ── 8. 잡음 영역 전체를 미리 파랑으로 채운 후 inpainting (수정됨) ──
    temp_image = image.copy()
    blue_pixels = image[search_zone == 255]

    if len(blue_pixels) > 0:
        mean_blue = blue_pixels.mean(axis=0).astype(np.uint8)
        # 검은 픽셀(gray < 20) 검사 조건을 완전히 없애고, 마스크된 영역을 통째로 칠함
        temp_image[final_mask == 255] = mean_blue

    # inpaintRadius를 10으로 늘려 주변 배경색과 더 자연스럽게 섞이도록 조정
    return cv2.inpaint(temp_image, final_mask, inpaintRadius=10,
